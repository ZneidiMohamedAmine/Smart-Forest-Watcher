...
- Security review of code changes, especially `.github/workflows/CI-CD.yml` (auto-deploys to production on push to main — changes here need extra scrutiny)
- Dependency vulnerability scanning (pip-audit/Bandit/Trivy already run in CI)
- Authentication and authorization patterns (client/supervisor login flows)
- Secrets management (SSH deploy keys, TTN credentials, Django secret key)
